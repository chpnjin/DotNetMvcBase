...
namespace WebBase.Models
{
    ...
    /// 職稱:繼承資料存取通用介面
    /// </summary>
    public class Title : ISqlCreator
    {
        public string MasterTable { get { return "HR_TITLE"; } }
        ...
        public string Search(JObject parm, bool getCount)
        {
            ...
            if (getCount)
            {
                sqlStr = string.Format(@"SELECT COUNT(*) `Count` FROM {0} T ", MasterTable);
            }
            else
            {
                sqlStr = string.Format(@"SELECT T.GUID,T.ID,T.NAME,T.REMARK,T.INSERT_TIME FROM {0} T ", MasterTable);
            }

            sqlStr += "WHERE T.ENABLE = 1 ";

            //代碼
            if (!string.IsNullOrEmpty((string)conditions.ID))
            {
                sqlStr += "AND T.ID LIKE CONCAT(@ID , '%') ";
            }
            //名稱
            if (!string.IsNullOrEmpty((string)conditions.NAME))
            {
                sqlStr += "AND T.NAME LIKE CONCAT(@NAME , '%') ";
            }
            //建立時間-起
        ...
        {
            string sqlStr = string.Format(@"SELECT T.GUID,T.ID,T.NAME,T.REMARK FROM {0} T ", MasterTable);
        ...
        {
            string sqlStr = string.Format(@"INSERT INTO {0} (GUID,ID,NAME,REMARK,INSERT_USER) ", MasterTable);
